                    ...
                    <table class="table table-striped table-vcenter">
                        ...
                        <tbody>
                            @foreach ($demands as $demand)
                                ...
                                <tr>
                                    <td>{{ $demand->id }}</td>
                                    <td>{{ $demand->created_at ? \Carbon\Carbon::parse($demand->created_at)->format('d/m/Y H:i:s') : '---' }}
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group btn-group-sm">
                                            ...
                                            @if ($demand->state == 1 && $demand->visa)
                                                <a href="{{ route('work-visas.show', $demand->visa) }}"
                                                    class="btn btn-sm btn-outline-secondary d-flex align-items-center border-0"
                                                    data-toggle="tooltip" title="Afficher visa">
                                                    <i class="fa fa-eye" aria-hidden="true"></i>
                                                    <span class="ml-2">Visa </span>
                                                </a>
                                            @elseif($demand->state < 0 && $demand->state !== -2)
                                            ...
                                            @endif
                                            {{-- @if (($demand->state >= 0 || $demand->state == -2) && $userEntity->id == $demand->dealing_structure && $user->role == 'directeur')
                                                @can('directeur-privilege')
                                                    <a href="{{ route('demande.edit.general', $demand) }}"
                                                        class="btn btn-sm btn-warning d-flex align-items-center border-0"
                                                        data-toggle="tooltip" title="Poursuivre l'enregistrement">
                                                        <i class="fa fa-pencil"></i>
